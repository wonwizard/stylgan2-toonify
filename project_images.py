...
#lnw add. for seepreety laten direction mov
...
def project_image(proj, src_file, dst_dir, tmp_dir, video=False):
    ...
    #lnw add
    filename = os.path.join(dst_dir, os.path.basename(src_file)[:-4] + '.npz')
    np.savez(filename, dlatents=proj.get_dlatents())

    #lnw add. for seepreety laten direction mov
    # Save latent
    #txt_filename = os.path.join(dst_dir, os.path.basename(src_file)[:-4] + '.txt')
